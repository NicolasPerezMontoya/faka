...
const CHANNELS: ChannelMeta[] = [
  ...
  { value: 'mercadolibre', label: 'Mercado Libre', hint: 'Pedidos + productos' },
  ...
  { value: 'falabella', label: 'Falabella', hint: 'Deshabilitado en F1', disabled: true, disabledReason: 'Fase 6' },
];
...
export function StepSource({ profiles, defaultChannel, defaultTipo, defaultProfileId }: StepSourceProps) {
  ...
  const [profileId, setProfileId] = React.useState<string | null>(defaultProfileId);

  const matchingProfiles = profiles.filter((p) => p.canal === channel && p.tipo === tipo);
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-1">1 · ¿De qué canal y qué tipo es este CSV?</h2>
      <p className="text-sm text-muted-foreground mb-5">
        El payload crudo se almacena en <code className="text-xs">raw_csv_uploads</code>; reprocesable con cualquier versión del perfil.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    selected ? 'border-primary ring-2 ring-primary/10' : 'border-input',
                    ch.disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-primary',
                  ].join(' ')}
                >
        ...
        <div className="space-y-5">
              ...
              {TIPOS.map((t) => (
                <label
                  ...
                  className={[
                    ...
                    tipo === t ? 'border-primary ring-2 ring-primary/10' : 'border-input hover:border-primary',
                  ].join(' ')}
                >
                  <input
                    type="radio"
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Perfil de mapping</label>
            <Select
              value={profileId ?? ''}
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Selecciona un perfil existente para auto-completar el mapeo, o créalo en el siguiente paso.
            </p>
          </div>

          {channel === 'falabella' && (
            <Badge variant="warn">Canal deshabilitado por feature flag (F6).</Badge>
          )}
        </div>
      </div>
