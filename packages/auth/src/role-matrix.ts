...
export function can(capability: Capability, role: UserRole | null | undefined): boolean {
 ...
 * authenticated user.
 */
export const ROUTE_ROLE_REQUIREMENTS: Record<string, ReadonlyArray<UserRole>> = {
  '/admin':              ['super_admin'],
 ...
 */
export function isPathAllowed(path: string, role: UserRole | null | undefined): boolean {
  if (!role) return false;
  let bestPrefix = '';
