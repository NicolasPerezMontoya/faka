...
export async function authMiddleware(request: NextRequest): Promise<NextResponse> {
  ...
  const role: UserRole | null =
    (user.app_metadata?.role as UserRole | undefined) ?? extractRole(user as never) ?? null;
...
export const DEFAULT_MATCHER = ['/((?!_next/static|_next/image|favicon.ico|public/|api/health).*)'];
