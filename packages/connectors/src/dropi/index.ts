...
export const createDropiConnector: ConnectorFactory<DropiConnectorConfig> = (_config) => {
  ...
  const connector: ChannelConnector = {
    ...
    async fetchOrders(_since: Date, _ctx: ConnectorContext): Promise<RawOrder[]> {
      throw new Error('NOT_IMPLEMENTED_F4: Dropi — CSV via CSVConnector is primary; scraper deferred');
    },

    async fetchProducts(_since: Date, _ctx: ConnectorContext): Promise<RawProduct[]> {
      throw new Error('NOT_IMPLEMENTED_F4: Dropi products sync via CSVConnector — Phase 4');
    },

    async normalizeOrder(_raw: RawOrder, _ctx: ConnectorContext): Promise<NormalizedOrder> {
      throw new Error('NOT_IMPLEMENTED_F4: Dropi order normalization — Phase 4 via CSVConnector');
    },

    async normalizeProduct(_raw: RawProduct, _ctx: ConnectorContext): Promise<NormalizedProduct> {
      throw new Error('NOT_IMPLEMENTED_F4: Dropi product normalization — Phase 4 via CSVConnector');
    },
