...
export const createPOSConnector: ConnectorFactory<POSConnectorConfig> = (_config) => {
  ...
  const connector: ChannelConnector = {
    ...
    async fetchProducts(_since: Date, _ctx: ConnectorContext): Promise<RawProduct[]> {
      throw new Error('NOT_IMPLEMENTED_F3: POS products sync — Phase 3 via webhook+CSV');
    },
    ...
    async normalizeProduct(_raw: RawProduct, _ctx: ConnectorContext): Promise<NormalizedProduct> {
      throw new Error('NOT_IMPLEMENTED_F3: POS product normalization — Phase 3');
    },
