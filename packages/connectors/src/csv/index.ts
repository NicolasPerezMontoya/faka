...
export const createCSVConnector: ConnectorFactory<CSVConnectorConfig> = (
  ...
  async function loadUpload(
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data)
      throw new Error(`upload_not_found: ${error?.message ?? uploadId}`);
  ...
  async function loadProfile(
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data)
      throw new Error(`profile_not_found: ${error?.message ?? profileId}`);
  ...
  async function upsertProducts(
    ...
    const { data: inserted, error: masterErr } = await supabase
      ...
      .select("master_sku");
    if (masterErr)
      throw new Error(`master_products_insert_failed: ${masterErr.message}`);
    ...
    if (mapErr)
      throw new Error(`product_mappings_upsert_failed: ${mapErr.message}`);
  }
  ...
  async function markRowsProcessed(
    ...
    if (error)
      throw new Error(`raw_csv_rows_processed_update_failed: ${error.message}`);
  }
  ...
  const connector: ChannelConnector & {
    ...
    async normalizeOrder(
      ...
      const profileColumnMap = (raw.payload_json._column_map ?? {}) as Record<
        string,
        string
      >;
    ...
    extractCustomerHint(raw: RawOrder): CustomerHint | null {
      ...
      const email =
        typeof payload.customer_email === "string"
          ...
          : undefined;
      const doc =
        typeof payload.customer_doc === "string"
      ...
      return {
        ...
        document_id: doc,
        displayed_name: name,
        external_identifier_type: phone
          ? "phone"
          : email
            ? "email"
            : "document",
    ...
    async ingestUpload(
      ...
      if (!upload.mapping_profile_id) {
        throw new Error(
          "mapping_profile_missing: pick a profile before ingesting",
        );
      }
      ...
      const processed = await streamRows(supabase, uploadId, async (batch) => {
        ...
        for (const row of batch) {
          if (targetTipo === "orders" || targetTipo === "order_items") {
            const r = safeNormalizeOrder(
              row.payload_json,
              profile.column_map_json,
              profile.canal,
            );
            if ("_error" in r) {
              errors.push({
                row_number: row.row_number,
                field: r._error.field,
                message: r._error.message,
              });
              skippedCount++;
            } else {
              validOrders.push(r);
        ...
        try {
          if (validOrders.length > 0)
            await upsertOrders(supabase, validOrders, uploadId);
          if (validProducts.length > 0)
            await upsertProducts(supabase, validProducts);
          await markRowsProcessed(supabase, uploadId, rowsToMark);
        } catch (err) {
          logger.error("csv.ingestUpload.batch_failed", {
      ...
      await setStatus(supabase, uploadId, finalStatus, {
        row_count: processed,
        error_log_json:
          errors.length > 0 ? { errors: errors.slice(0, 100) } : null,
      });

      logger.info("csv.ingestUpload.done", {
        uploadId,
        processed,
        validCount,
        skippedCount,
