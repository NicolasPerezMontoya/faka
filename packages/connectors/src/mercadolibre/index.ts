...
export const createMercadoLibreConnector: ConnectorFactory<MercadoLibreConnectorConfig> = (_config) => {
  ...
  const connector: ChannelConnector = {
    ...
    async fetchOrders(_since: Date, _ctx: ConnectorContext): Promise<RawOrder[]> {
      throw new Error('NOT_IMPLEMENTED_F4: Mercado Libre OAuth + orders sync — Phase 4');
    },

    async fetchProducts(_since: Date, _ctx: ConnectorContext): Promise<RawProduct[]> {
      throw new Error('NOT_IMPLEMENTED_F4: Mercado Libre publications sync — Phase 4');
    },

    async normalizeOrder(_raw: RawOrder, _ctx: ConnectorContext): Promise<NormalizedOrder> {
      throw new Error('NOT_IMPLEMENTED_F4: Mercado Libre order normalization — Phase 4');
    },

    async normalizeProduct(_raw: RawProduct, _ctx: ConnectorContext): Promise<NormalizedProduct> {
      throw new Error('NOT_IMPLEMENTED_F4: Mercado Libre product normalization — Phase 4');
    },
