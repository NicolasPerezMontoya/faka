...
export const createWordPressConnector: ConnectorFactory<WordPressConnectorConfig> = (_config) => {
  ...
  const connector: ChannelConnector = {
    ...
    async fetchOrders(_since: Date, _ctx: ConnectorContext): Promise<RawOrder[]> {
      throw new Error('NOT_IMPLEMENTED_F2: WordPress orders sync — Phase 2 walking skeleton');
    },

    async fetchProducts(_since: Date, _ctx: ConnectorContext): Promise<RawProduct[]> {
      throw new Error('NOT_IMPLEMENTED_F2: WordPress products sync — Phase 2 walking skeleton');
    },

    async normalizeOrder(_raw: RawOrder, _ctx: ConnectorContext): Promise<NormalizedOrder> {
      throw new Error('NOT_IMPLEMENTED_F2: WordPress order normalization — Phase 2');
    },

    async normalizeProduct(_raw: RawProduct, _ctx: ConnectorContext): Promise<NormalizedProduct> {
      throw new Error('NOT_IMPLEMENTED_F2: WordPress product normalization — Phase 2');
    },
