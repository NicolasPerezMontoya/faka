...
export const Thead = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <thead ref={ref} className={cn('bg-muted text-xs uppercase tracking-wider text-muted-foreground', className)} {...props} />
  ),
);
...
export const Th = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th ref={ref} className={cn('px-4 py-2 text-left font-medium', className)} {...props} />
  ),
);
Th.displayName = 'Th';
