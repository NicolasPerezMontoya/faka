...
- [ ] **FND-02**: Supabase 5-layer schema deployed end-to-end (RAW + MASTER + FACTS + MARTS + INSIGHTS skeleton). RAW layer includes `raw_orders`, `raw_products`, `raw_events`, and the LOCKED tables `raw_csv_uploads`, `raw_csv_rows`, `csv_mapping_profiles` per CONSTR-raw-csv-schema. MASTER includes `master_products`, `product_mappings`, `product_variants`, **`master_categories` (jerárquica), `category_mappings`** (taxonomía maestra), **`customers`, `customer_external_links`, `customer_merge_log`** (Mini-CRM stubs, ADR-004 LOCKED — popula F4). FACTS includes `sales`, `sale_items` (con nullable `master_sku` y nullable `customer_id`), `inventory_snapshots`. MARTS skeleton scaffolded. INSIGHTS includes `ai_insights`, `ai_conversations`. **`messaging_log`** (vacía, ADR-003 LOCKED — popula F5.5).
- [ ] **FND-03**: Supabase Auth wired with **4 roles** (`super_admin`, `admin`, `manager`, `analista`) per ADR-002 LOCKED; RLS row-level policies on every user-readable table (CONSTR-rls-required); **vistas por rol con grants column-level** (`*_view_admin` / `*_view_manager` / `*_view_analista`) que esconden columnas $ a Analista y columnas cliente a Manager+Analista; JWT-claim middleware en Next.js valida y propaga rol; login UI con creación de Super Admin inicial vía CLI seeder (email `nicolasperezmontoya@gmail.com`).
