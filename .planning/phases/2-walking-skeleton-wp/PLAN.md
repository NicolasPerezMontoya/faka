...
- **Task:** Create `apps/dashboard/app/(app)/matching/[mappingId]/page.tsx`:
  ...
  - Renders: a 2-column grid (per RESEARCH §Don't Hand-Roll — Tailwind grid + `@faka/ui/Card`):
    ...
    - RIGHT column: "Candidato master" — `master_products.nombre_canonico`, brand, category, master*sku (read-only badge), the cascade method + score (`<Badge>` colored by tier), the LLM rationale (if `method='llm_arbiter*\*'`, fetched from `product_mappings.metadata_json->>rationale` — add this column if not present via additive migration).
...
| F2-CC-6  | No service-role / WP secrets in dashboard env or browser (CC-11) | `grep -E 'NEXT*PUBLIC*.\*(SERVICE\\                                                                                                                                                                                                    | SECRET\\                                                                                                                                            | PRIVATE\\                                                     | WORDPRESS\\                   | OPENAI\\ | MOONSHOT\\ | ANTHROPIC)' apps/dashboard/.env.example apps/dashboard/vercel.json` returns 0 matches | F2 SC4 + invariant CC-11 |
