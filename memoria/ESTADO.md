...
4. **CI VERDE ✓ Y ESTRICTO** desde commit `796540e` (run 25824861447). Ver `memoria/CI-FIXES.md` para la cascada completa.

### En el código (estado actual)

- **`packages/db/types/database.ts`** es el output verbatim de `supabase gen types`; excluido de prettier via `.prettierignore`. CI lo regenera y `git diff --exit-code` valida que esté al día. Si tocas migrations, corre `pnpm db:types` y commitea.
- **Format check** es estricto: cualquier archivo sin formato falla CI. Si pnpm está disponible, `pnpm format` antes de commit. Si no, podemos volver al truco del patch artifact (ver CI-FIXES.md).
- **Type check** es estricto end-to-end.

### Pendientes menores

1. Plans 1.2.5 + 1.4.3 (tests de integración) — scripts `test:integration` están noop. F2 escribe vitest configs reales contra Supabase live.
2. Generar `pnpm-lock.yaml` baseline → commitear (CI corre sin `--frozen-lockfile`).
3. ESLint dashboard noop. F2: alinear a ESLint 8.57.1 + eslint-config-next 14 (o bump a Next 15) y re-habilitar.
