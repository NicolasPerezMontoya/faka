...
const CHANNELS_TO_LOAD: Channel[] = [
  "wordpress",
  "mercadolibre",
  "dropi",
  "pos",
...
async function main(): Promise<void> {
  ...
  let llmCfg = resolveLLMConfig({
    cliProvider: cfg.cliProvider,
  ...
  if (cfg.enableLLMArbiter && llmCfg.provider !== "none") {
    ...
    console.warn(
      `⚠️  No LLM provider detected (no API keys in env). LLM arbiter disabled.`,
    );
    cfg.enableLLMArbiter = false;
  }
  ...
  if (!existsSync(cfg.inputDir)) {
    ...
    console.error(
      `   Create the directory structure described in scripts/discovery/README.md`,
    );
    process.exit(1);
  }
  ...
  for (const ch of CHANNELS_TO_LOAD) {
    try {
      const { products, files } = loadChannel(
        cfg.inputDir,
        cfg.profilesDir,
        ch,
      );
      if (products.length > 0) {
        ...
        console.log(
          `   ✓ ${ch}: ${products.length} products (${files.length} files)`,
        );
      } else {
      ...
      console.log(
        `   - ${ch}: skipped (${(err as Error).message.split("\n")[0]})`,
      );
    }
  }
  ...
  if (loaded.length < 2) {
    console.error(
      `❌ Need at least 2 channels with data. Loaded: ${loaded.length}`,
    );
    process.exit(1);
  }
  ...
  if (!anchorLoad) {
    console.error(
      `❌ Anchor channel '${cfg.anchorChannel}' has no products loaded.`,
    );
  ...
  const unresolved: Array<{
    ...
    topCandidates: CanonicalProduct[];
  }> = [];
  ...
  let llmCalls = 0;
  if (
    cfg.enableLLMArbiter &&
    unresolved.length > 0 &&
    llmCfg.provider !== "none"
  ) {
    const llmTargets = unresolved.slice(
      0,
      Math.min(cfg.maxLLMCalls, unresolved.length),
    );
    console.log(
    ...
    for (const u of llmTargets) {
      ...
      llmCalls++;
      matches.push(
        promoteToMatch({ anchor: u.anchor, candidate: cand }, decision),
      );
    }
  }
  ...
  matches_by_method.unresolved = Math.max(
    ...
    unresolved.length -
      (matches_by_method.llm_arbiter_match +
        matches_by_method.llm_arbiter_reject),
  );

  const nonAnchorTotal =
    loaded
  ...
  const automaticHits =
    ...
    matches_by_method.llm_arbiter_match;
  const reviewNeeded =
    matches_by_method.embeddings_mid +
    matches_by_method.unresolved +
    matches_by_method.llm_arbiter_reject;
  ...
  const report: DiscoveryReport = {
    ...
    config: {
      ...
      llmProvider:
        llmCfg.provider === "none"
          ? "openai"
          : (llmCfg.provider as "openai" | "anthropic"),
    ...
    inputs: loaded.flatMap((l) =>
      l.files.map((f) => ({
        channel: l.channel,
        file: f,
        row_count: l.products.length / l.files.length,
      })),
    ),
    totals_by_channel,
    matches_by_method,
    ...
    unresolved_samples: unresolved.slice(0, 20).map((u) => ({
      ...
      candidates: u.topCandidates
        .slice(0, 3)
        .map((c) => `${c.channel}:${c.external_id} — ${c.name}`),
    })),
  ...
  console.log(`\n✅ Done.`);
  console.log(
    `   Automatic match rate: ${(report.match_rate_automatic * 100).toFixed(1)}%`,
  );
  console.log(
    `   Review queue size:    ${(report.match_rate_review_needed * 100).toFixed(1)}%`,
  );
...
function buildRecommendation(
  ...
  const reviewQueue =
    byMethod.embeddings_mid + byMethod.unresolved + byMethod.llm_arbiter_reject;
  const llmContribution = byMethod.llm_arbiter_match;
  const llmRejectRate =
    byMethod.llm_arbiter_match + byMethod.llm_arbiter_reject > 0
      ? byMethod.llm_arbiter_reject /
        (byMethod.llm_arbiter_match + byMethod.llm_arbiter_reject)
      : 0;
